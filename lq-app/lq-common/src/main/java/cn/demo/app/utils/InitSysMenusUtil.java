package cn.demo.app.utils;

import cn.demo.app.sys.modules.models.*;
 ...
 * @time : 2019/1/17-21:24
 * @Description : 初始化菜单
 **/
public class InitSysMenusUtil {
     ...
     * @param dao
     */
    public void initSys(Dao dao){
        //这些表不需要自动创建
        final Set<Class<?>> tableEntitys = new HashSet<>();
        tableEntitys.addAll(
                Arrays.asList(
                    Sys_role.class,
                    Sys_menu.class,
                    ...
                    Sys_user.class));
        try {
                Daos.createTablesInPackage(dao, "cn.demo.app.sys", false,(aClass, table) -> !tableEntitys.contains(aClass));
                //统一据库表结构-不删除已有字段避免已经创建的动态字段被删
                //统一据库表结构-不删除已有字段避免已经创建的动态字段被删
                for (Class<?> klass : Scans.me().scanPackage("cn.demo.app.sys")) {
                    if (klass.getAnnotation(Table.class) != null && !tableEntitys.contains(klass)) {
                        Daos.migration(dao, klass, true, false, true, null);
                    }
                }
        } catch (Exception e) {
        }
        if (0 == dao.count(Sys_user.class)) {
            ...
            unit.setEmail("demo@qq.com");
            unit.setTelephone("");
            unit.setHasChildren(false);
            unit.setParentId("");
            unit.setWebsite("https://demo.cn");
     ...
     * @return
     */
    private void initSysConfig(Dao dao){
        List<Sys_config> sysConfigs = new ArrayList<>();
        sysConfigs.add(new Sys_config().setConfigKey("AppName").setConfigValue("Nutz-B-LQ").setNote("系统名称"));
        sysConfigs.add(new Sys_config().setConfigKey("AppShrotName").setConfigValue("Nutz-LQ").setNote("系统短名称"));
        sysConfigs.add(new Sys_config().setConfigKey("AppDomain").setConfigValue("http://127.0.0.1").setNote("系统域名"));
        sysConfigs.add(new Sys_config().setConfigKey("AppUploadPath").setConfigValue("D://files/upload").setNote("文件上传文件夹"));
        sysConfigs.add(new Sys_config().setConfigKey("AppUploadBase").setConfigValue("/upload").setNote("文件访问路径"));
        sysConfigs.add(new Sys_config().setConfigKey("SysUserSessionOnlyOne").setConfigValue("true").setNote("用户登录只允许一个Session实例(为true时退出登录会更新sys_user表在线状态)"));
        sysConfigs.add(new Sys_config().setConfigKey("WebNotification").setConfigValue("true").setNote("启用浏览器通知"));
        dao.insert(sysConfigs);
    }
    /**
     * //初始化菜单
     * @param dao
     * @return
     */
    private void initSysMenu(Dao dao){
